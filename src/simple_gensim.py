...
from numpy import exp, log, dot, zeros, outer, random, dtype, float32 as REAL,\
    ...
    ndarray, empty, sum as np_sum, prod, ones, ascontiguousarray, argpartition, argsort, asarray

class matutils:
    def unitvec(vec, norm='l2'):
        return vec/sqrt(np_sum(vec**2))
    ...
    def argsort(x, topn=None, reverse=False):
        ...
        if topn >= x.size: # or not hasattr(np, 'argpartition'):
            return argsort(x)[:topn]
        most_extreme = argpartition(x, topn)[:topn]
        return most_extreme.take(argsort(x.take(most_extreme)))
